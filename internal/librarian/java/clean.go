...
// It scans the file using fast byte scanning without per-line allocations and
// returns true as soon as the marker is found, avoiding reading the entire file.
func hasMarker(path string) (bool, error) {
	...
	buf := make([]byte, 8192)
	overlap := 0
	maxMarkerLen := max(len(autoGeneratedMarkerBytes), len(autoGeneratedAnnotationBytes))

	for {
		n, rerr := f.Read(buf[overlap:])
		total := overlap + n
		if total > 0 {
			window := buf[:total]
			if bytes.Contains(window, autoGeneratedMarkerBytes) || bytes.Contains(window, autoGeneratedAnnotationBytes) {
				return true, nil
			}
		}
		if rerr != nil {
		...
		if total >= maxMarkerLen {
			copy(buf, buf[total-maxMarkerLen+1:total])
			overlap = maxMarkerLen - 1
		} else {
			overlap = total
		}
	}
